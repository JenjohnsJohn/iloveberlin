...
async function getTopics(): Promise<TopicData[]> {
  try {
    const res = await fetch(`${API_URL}/guides/topics`, {
      next: { revalidate: 300 },
    });
    if (res.ok) {
...
function TopicIcon({ icon }: { icon: string | null }) {
  ...
  switch (resolved) {
    ...
    case 'MAP':
      return (
        <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 6.75V15m0-8.25a1.5 1.5 0 010 3m0-3a1.5 1.5 0 000 3m0 9.75V15m0 0a1.5 1.5 0 110 3m0-3a1.5 1.5 0 100 3m-3.375 0h10.5a1.5 1.5 0 001.5-1.5V5.625a1.5 1.5 0 00-1.5-1.5H5.625a1.5 1.5 0 00-1.5 1.5v12.75a1.5 1.5 0 001.5 1.5z" />
        </svg>
      );
    case 'PEOPLE':
      return (
