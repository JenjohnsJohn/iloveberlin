...
export default function GuideTopicsPage() {
  ...
  const fetchTopics = useCallback(async () => {
    try {
      ...
      const list = Array.isArray(data) ? data : (data.data ?? data.topics ?? []);
      setTopics(list);
    } catch (err: unknown) {
      const message =
        err instanceof Error ? err.message : 'Failed to load guide topics';
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  // Build a flat display list with smart search and orphan handling
  const buildDisplayList = (): Array<{ topic: GuideTopic; isChild: boolean; childCount: number }> => {
    const result: Array<{ topic: GuideTopic; isChild: boolean; childCount: number }> = [];

    if (!searchQuery.trim()) {
      for (const parent of parentTopics) {
        const children = childrenOf(parent.id);
        result.push({ topic: parent, isChild: false, childCount: children.length });
        for (const child of children) {
          result.push({ topic: child, isChild: true, childCount: 0 });
        }
      }
      // Orphaned children (parent_id set but parent not in list)
      const parentIds = new Set(parentTopics.map((p) => p.id));
      topics
        .filter((t) => t.parent_id && !parentIds.has(t.parent_id))
        .sort((a, b) => a.sort_order - b.sort_order)
        .forEach((orphan) => result.push({ topic: orphan, isChild: true, childCount: 0 }));
      return result;
    }
    ...
    for (const parent of parentTopics) {
      ...
      if (parentMatches || matchingChildren.length > 0) {
        result.push({ topic: parent, isChild: false, childCount: allChildren.length });
  ...
  const openEditModal = (topic: GuideTopic) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingTopic) {
        ...
        await apiClient.patch(`/guides/topics/${editingTopic.id}`, payload);
      } else {
        // Compute sort_order for new topic based on siblings
        ...
        await apiClient.post('/guides/topics', payload);
      }
      ...
      const message =
        err instanceof Error ? err.message : 'Failed to save guide topic';
      setError(message);
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async (id: string) => {
    try {
      setError(null);
      await apiClient.delete(`/guides/topics/${id}`);
      setDeleteConfirm(null);
      await fetchTopics();
    } catch (err: unknown) {
      const message =
        err instanceof Error ? err.message : 'Failed to delete guide topic';
      setError(message);
    }
  };

  const getSiblings = (topic: GuideTopic) =>
    topic.parent_id === null ? parentTopics : childrenOf(topic.parent_id);

  const isFirst = (topic: GuideTopic) => {
    const siblings = getSiblings(topic);
    return siblings.length === 0 || siblings[0].id === topic.id;
  };

  const isLast = (topic: GuideTopic) => {
    const siblings = getSiblings(topic);
    return siblings.length === 0 || siblings[siblings.length - 1].id === topic.id;
  };

  const moveSibling = async (topic: GuideTopic, direction: 'up' | 'down') => {
    const siblings = getSiblings(topic);
    ...
    const swapTopic = siblings[swapIndex];
    const currentOrder = topic.sort_order;
    const swapOrder = swapTopic.sort_order;

    // Optimistic update
    setTopics((prev) =>
      prev.map((t) => {
        if (t.id === topic.id) return { ...t, sort_order: swapOrder };
        if (t.id === swapTopic.id) return { ...t, sort_order: currentOrder };
        return t;
      })
    );
    ...
    try {
      setError(null);
      await apiClient.patch(`/guides/topics/${topic.id}`, { sort_order: swapOrder });
      await apiClient.patch(`/guides/topics/${swapTopic.id}`, { sort_order: currentOrder });
      await fetchTopics();
    } catch (err: unknown) {
      const message =
        err instanceof Error ? err.message : 'Failed to reorder topics';
      setError(message);
      await fetchTopics();
    }
  };
  ...
  const collapseAll = () => setCollapsed(new Set(parentTopics.map((p) => p.id)));
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Guide Topics</h2>
          {!loading && (
            <p className="text-sm text-gray-500 mt-0.5">
              {displayList.length} {displayList.length === 1 ? 'topic' : 'topics'}
              {searchQuery && ' found'}
            </p>
          )}
        </div>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        {parentTopics.length > 0 && !searchQuery && (
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12">
          ...
          <span className="text-sm text-gray-500">Loading guide topics...</span>
        </div>
        ...
        <div className="space-y-2">
          {parentTopics
            .filter((parent) => {
              ...
              return childrenOf(parent.id).some((c) => c.name.toLowerCase().includes(q));
            })
            .map((parent) => {
              const children = (() => {
                const all = childrenOf(parent.id);
              ...
              return (
                ...
                >
                  {/* Parent row */}
                  <div
                    className={`group flex items-center gap-2 px-3.5 py-2.5 transition-colors hover:bg-gray-50 ${
                  ...
                  >
                    ...
                    >
                      <button
                        onClick={() => moveSibling(parent, 'up')}
                        disabled={isFirst(parent)}
                      ...
                      <button
                        onClick={() => moveSibling(parent, 'down')}
                        disabled={isLast(parent)}
                        className="text-gray-400 hover:text-gray-600 disabled:opacity-20 disabled:cursor-not-allowed"
                        title="Move down"
                      >
                        <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {(parent.guide_count ?? 0) > 0 && (
                          <span className="text-xs text-gray-400">{parent.guide_count} guides</span>
                        )}
                      </div>
                    ...
                    <div className="border-t border-gray-100">
                      <div className="ml-5 border-l-2 border-primary-100 bg-gray-50/40">
                        {children.map((child) => (
                          <div
                            key={child.id}
                            className="group flex items-center gap-2 px-3 py-2 border-b border-gray-100/80 last:border-b-0 hover:bg-white/60 transition-colors"
                          >
                            ...
                            <div className="flex flex-col items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                              ...
                              <button
                                onClick={() => moveSibling(child, 'down')}
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                ...
                                {(child.guide_count ?? 0) > 0 && (
                                  <span className="text-xs text-gray-400">{child.guide_count} guides</span>
                                )}
                              </div>
                  ...
                  {hasChildren && !isExpanded && (
                    ...
                    >
                      {children.length} subtopic{children.length === 1 ? '' : 's'} hidden
                    </button>
                  )}
                </div>
              );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md p-4 z-10">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-2.5 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm"
                  placeholder="Icon (e.g. emoji or icon name)"
                />
              </div>
            ...
            <div className="flex justify-end space-x-3 mt-6">
              ...
              >
                {saving ? 'Saving...' : editingTopic ? 'Save Changes' : 'Create'}
              </button>
            </div>
          </div>
        </div>
