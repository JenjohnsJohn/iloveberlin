...
class VideoPipeline(BasePipeline):
    ...
    async def fetch(self) -> list[RawItem]:
        # Read queries and max_results from DB, update source if available
        db_queries = await get_json_setting("source.youtube_queries", [])
        if db_queries:
            self.source.search_queries = db_queries
        max_results = await get_int_setting(
            "target.videos.max_results_per_query",
            self.source.max_results_per_query,
        )
        self.source.max_results_per_query = max_results
