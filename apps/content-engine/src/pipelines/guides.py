...
class GuidePipeline(BasePipeline):
    ...
    async def fetch(self) -> list[RawItem]:
        self.daily_count = await get_int_setting(
            "target.guides.daily_count", self.daily_count
        )

        # Phase 3.1: read topics from DB, fall back to hardcoded defaults
        topics = await get_json_setting("source.ai_guide_topics", DEFAULT_GUIDE_TOPICS)
        if not topics:
            topics = DEFAULT_GUIDE_TOPICS

        items = []
        today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        day_of_year = datetime.now(timezone.utc).timetuple().tm_yday
        for i in range(self.daily_count):
            idx = (day_of_year * self.daily_count + i) % len(topics)
            topic = topics[idx]
