...
async def main():
    ...
    default_settings = [
        ...
        ("schedule.articles_rss", "0 6,8,10,12,14,16,18,20,22 * * *", "Cron schedule for Articles (RSS)"),
        ("schedule.articles_ai", "0 5 * * *", "Cron schedule for Articles (AI)"),
        ("schedule.events_berlinde", "0 6,14 * * *", "Cron schedule for Events (berlin.de)"),
        ("schedule.events_ai", "30 5 * * *", "Cron schedule for Events (AI)"),
        ("schedule.restaurants", "0 3 * * *", "Cron schedule for Restaurants"),
        ("schedule.guides", "0 4 * * *", "Cron schedule for Guides"),
        ("schedule.videos", "0 8,16 * * *", "Cron schedule for Videos"),
        ("schedule.competitions", "0 6 */3 * *", "Cron schedule for Competitions"),
